...
class HotelsBuilder private constructor() {
    ...
    fun withHotelId(id: Long): HotelsBuilder = this.apply {
        this.hotel = hotel.copy(id = id)
    }

    fun withVat(vat: BigDecimal): HotelsBuilder = this.apply {
        this.hotel = hotel.copy(vat = vat)
    }

    fun withCurrency(currency: Currency): HotelsBuilder = this.apply {
        this.hotel = hotel.copy(currency = currency.currencyCode)
    }

    fun withRooms(vararg rooms: RoomsBuilder): HotelsBuilder = this.apply {
        this.rooms = rooms.toList()
    }

    fun withExtraCharge(vararg extraChargeFlat: ExtraChargeFlatBuilder): HotelsBuilder = this.apply {
        this.extraChargeFlat = extraChargeFlat.toList()
    }

    fun withExtraCharge(vararg extraChargePercentage: ExtraChargePercentageBuilder): HotelsBuilder = this.apply {
        this.extraChargePercentage = extraChargePercentage.toList()
    }

    fun build(): HotelDTO = HotelDTO(
        hotel = hotel,
