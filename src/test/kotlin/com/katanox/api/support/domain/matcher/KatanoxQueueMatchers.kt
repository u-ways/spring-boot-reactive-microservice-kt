...
class KatanoxQueueMatchers(
    ...
    fun Supplier<Message?>.shouldNotContainAnyMessages() =
        generateSequence(::get)
            .toList()
    ...
    infix fun Supplier<Message?>.shouldContainExactly(block: BookingDTOBuilder.() -> Unit): Message {
        ...
        if (messages.size != 1) {
            fail("Expected queue to contain exactly one message, but received: $messages")
        } else {
            JSON_MAPPER
    ...
    infix fun Message.withProperties(block: MessageProperties.() -> Unit): MessageProperties = messageProperties.apply(block)
}
